...
主程序使用 `TIM2_CH1` 在 `PA0` 上输出 PWM 波形，并在 OLED 屏幕上动态显示当前 PWM 占空比。
项目由 STM32CubeMX 生成 CMake 构建配置，并结合自定义 OLED 显示模块。

## 主要功能

- 使用 STM32 HAL 初始化 STM32F103 MCU
- 配置 TIM2 为 PWM 模式，并在 `PA0` 输出 PWM 信号（TIM2_CH1）
- 通过 OLED 实时显示当前 PWM 值
- OLED 采用软件 I2C 驱动，支持清屏、显示字符串和数字
...
```bash
cd d:/Electronics/HAL_Projects/HAL_4_TIM_PWM_LED
cmake --preset Debug
...
2. 重新上电后，OLED 应显示当前 PWM 值，TIM2_CH1 的 `PA0` 引脚会输出对应的 PWM 波形。
